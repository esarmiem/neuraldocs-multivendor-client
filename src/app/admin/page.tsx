...
export default function AdminPage() {
  ...
  const handleClearDatabase = async () => {
    if (
      window.confirm(
        "¿Estás seguro de que quieres limpiar la base de datos? Esta acción es irreversible.",
      )
    ) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                {documents.length > 0 ? (
                  <ul className="space-y-2">
                    {documents.map((doc, index) => (
                      <li
                        key={index}
                        className="flex items-center text-gray-700 bg-gray-50 p-2 rounded-md"
                      >
          ...
          <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 hover:border-red-500 transition-all">
            ...
            <p className="text-gray-500 mb-4 text-sm">
              Esta acción eliminará todos los documentos y chunks de la base de
              datos vectorial.
            </p>
            <button
